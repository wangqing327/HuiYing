 ...
 */
class DownloadFragment : Fragment(), Observer<MutableList<DownLoadFileInfo>>,
    ...
    private lateinit var mAdapter: AdapterDownload
    ...
    override fun onCreateView(
        ...
        mAdapter = AdapterDownload(R.layout.historylayout)
        ...
        when (index) {
            0 -> {
                pageViewModel.videoLinks.observe(requireActivity(),this)
            }
            1 -> {
                /*pageViewModel.videoLinks.observe(requireActivity(), this::onChanged)
                lifecycleScope.launch(Dispatchers.IO) {
                    val data = dao.queryAllData(index)
                    pageViewModel.videoLinks.postValue(data)
                }*/
            }
        }
    ...
    companion object {
         ...
         */
        @JvmStatic
        fun newInstance(sectionNumber: Int): DownloadFragment {
    ...
    override fun onChanged(t: MutableList<DownLoadFileInfo>?) {
    ...
    override fun onItemClick(adapter: BaseQuickAdapter<*, *>, view: View, position: Int) {
        if (edit) {
            ...
            val item = adapter.getItem(position) as DownLoadFileInfo
            if (TbsVideo.canUseTbsPlayer(requireActivity())) {
                TbsVideo.openVideo(requireActivity(), item.link)
            }
