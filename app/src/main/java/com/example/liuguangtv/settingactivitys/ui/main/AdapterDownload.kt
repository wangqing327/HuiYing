 ...
 * 记录与收藏页面适配器
 */
class AdapterDownload(layoutResId: Int) :
    BaseItemDraggableAdapter<DownLoadFileInfo, BaseViewHolder>(layoutResId, null) {
    /**
     * 当前视图checkBox是否可显示
     */
    ...
    override fun convert(helper: BaseViewHolder, item: DownLoadFileInfo) {
        helper.setText(R.id.title, item.name)
        helper.setText(R.id.url, item.link)
     ...
     * 取项目中所有已选择的项目集合
     */
    fun getAllCheckedItems(): MutableList<DownLoadFileInfo> {
        val lists = mutableListOf<DownLoadFileInfo>()
        data.forEach {
            if (it.checked) {
                lists.add(it)
            }
